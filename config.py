...
class PlanConfig:
    ...
    max_distinct_kisakod_repeatable: Optional[int] = None  # Kaç farklı KisaKod tekrarlı olarak kullanılabilir
    ...
    prioritize_never_used_first: bool = True  # Prioritize products never used as FIRST
    ...
    # Zorla kullanılacak tercihli ürünler (kriterleri sağlamasa bile)
    forced_preferred_products: List[str] = field(default_factory=list)  # List of kisakodrenk (uppercase)
    
    # Havuza eklenen tercihli ürünler (kriterlere uygun olsun ya da olmasın)
    preferred_products_in_pool: List[str] = field(default_factory=list)  # List of kisakodrenk (uppercase)
    ...
    def to_dict(self) -> dict:
        ...
        for size_count in sorted(self.front_size_stock_rules.keys()):
            ...
            # Save all rules - preserve user input even if values are 0
            # This ensures that values entered by user (like for size 4 and 6) are preserved
            # during pool calculations and auto-save operations
        ...
        return {
            ...
            "max_distinct_kisakod_repeatable": self.max_distinct_kisakod_repeatable or 0,
            ...
            "preferred_products_in_pool": [p.upper() for p in self.preferred_products_in_pool],
            "forced_preferred_products": [p.upper() for p in self.forced_preferred_products],
        }
